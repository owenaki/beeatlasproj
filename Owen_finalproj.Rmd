...
ggplot(data =genuslist[1:pmax(length(genuslist),10),], aes(x=reorder(Genus, -n), y=n, fill =Genus)) +
...
ggplot(data =specieslist[1:pmax(length(specieslist),25),], aes(x=reorder(gspecies, -n), y=n, fill =gspecies)) +
  geom_bar(stat='identity',show.legend = FALSE)+
...
```{r}
#split into genus column
#OBA_onetype$plant.genus<-str_split_fixed(OBA_onetype$Associated.plant...genus..species, "\\s",n=2)[,1]

plantlist_g<-OBA_onetype %>% group_by(plant.genus) %>% tally()
plantlist_g<-plantlist_g[order(plantlist_g$n, decreasing = TRUE),1:2]

ggplot(data =plantlist_g[1:pmax(length(plantlist_g$plant.genus),20),], aes(x=reorder(plant.genus, -n), y=n, fill =plant.genus)) +
  geom_bar(stat='identity',show.legend = FALSE)+
...
ggplot(data =plantlist[1:pmax(length(plantlist$Associated.plant...genus..species),25),], aes(x=reorder(Associated.plant...genus..species, -n), y=n, fill =Associated.plant...genus..species)) +
  geom_bar(stat='identity',show.legend = FALSE)+
...
```{r}
#remove any bee species only identified to genus
OBA_onetype_plantgenus_d<-subset(OBA_onetype, plantresolution == TRUE & Species != "")
...
for (a in 1:nrow(allplants_g)) {
    oneplantsubset_g<-subset(OBA_onetype_plantspecies_d, plant.genus == allplants_g[a,1])
    allplants_g[a,2]<-length(unique(oneplantsubset_g$gspecies))
}


ggplot(data =allplants_g[1:pmax(length(allplants_g),25),], aes(x=reorder(plant, -uniquebeespecies), y=uniquebeespecies, fill =plant)) +
  geom_bar(stat='identity',show.legend = FALSE)+
