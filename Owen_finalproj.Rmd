...
OBA_onetype<-OBA_crop[which(vegbee_master$to == onevegtype),] 

sprintf("This point is within %s vegetation cover",onevegtype)

gsub(" ","+",onevegtype)
browseURL("https://www.google.com/search?q=Developed,+Low+Intensity+site%3Ahttps%3A%2F%2Fexplorer.natureserve.org")
#a function that doesn't work #toplist <- function(x){ # funclist<-OBA_greasewood %>% group_by (x) %>% tally() # funclist<-random[order(funclist$n, decreasing = TRUE),1:2] #return(funclist) }
```
